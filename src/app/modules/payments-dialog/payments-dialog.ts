...
export class PaymentsDialog implements OnChanges {
  ...
  onKey(e: KeyboardEvent) {
    ...
    if (e.key === 'F2') {
      this.addRowQuick('CASH');
      e.preventDefault();
    }
    if (e.key === 'F3') {
      this.addRowQuick('CARD');
      e.preventDefault();
    }
    if (e.key === 'F4') {
      this.split50();
  ...
  private get activeRow(): Row {
    return this.rows[this.activeIndex];
  }
  ...
  allCard() {
    this.rows = [{method: 'CARD', amount: this.total, externalTxnId: null}];
    this.activeIndex = 0;
    this.recalc();
  }
