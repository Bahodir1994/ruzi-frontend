...
@Component({
  ...
  templateUrl: './item.library.html',
  standalone: true,
  styleUrl: './item.library.scss',
  encapsulation: ViewEncapsulation.None
})
export class ItemLibrary implements OnInit {
  ...
  permission(status: string) {
    this.permissions = {
      add_new_product: this.permissionService.canAccess(ItemLibrary, 'add_new_product', status)
    };
  }
  ...
  async loadData() {
    ...
    try {
      ...
      this.productModel = data.data as ItemModel[];
